...
from dtw import accelerated_dtw  # Using dtw package
...
class PunjabiPronunciationScorer:
    ...
    def compute_wer_similarity(self, audio_path: str, reference_text: str) -> float:
        ...
        try:
            ...
            # WER of 0% = 100% accuracy, WER >= 100% = 0% accuracy
        ...
        try:
            ...
            if len(silent_frames) > 0:
                ...
                if len(silence_starts) > 0 and len(silence_ends) > 0:
                    ...
                    # Penalize excessive silence or very short utterings
                    ...
                    silence_penalty = min(silary, 1.0)  # Cap at 1.0
                else:
                    silence_penalty = 0.0
            else:
                silence_penalty = 0.0
    ...
    def score_pronunciation(self, 
        ...
        try:
            ...
            if reference_audio_path and os.path.exists(reference_audio_path):
                ...
                details['detected_tone'] = user_tone_detected
                
                if expected_tone and user_tone_detected != 'unknown':
                    # Simple tone matching score
                    tone_score = 100.0 if user_tone_detected == expected_tone else 40.0
                else:
